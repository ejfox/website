<template>
  <main class="pt-16 dark:bg-gray-900 p-4">
    <ContentQuery path="/blog/" :sort="{ date: -1 }" v-slot="{ data }" :limit="queryLimit"
      :where="{ title: { $not: /!/ } }" :skip="querySkip">
      ...
      <div v-if="data">
        ...
        <div class="my-10 md:my-2 min-h-min pb-8">
          ...
          <div class="relative z-10 px-4 sm:px-6 lg:px-8 lg:py-40">
            ...
            <h2 class="text-sm font-semibold text-slate-500 tracking-wide uppercase py-1 md:py-2 px-2">featured
            </h2>

            <h1 class="text-4xl font-extrabold tracking-tight dark:text-white sm:text-5xl lg:text-9xl">
              {{ featuredArticle(data).title }}
            </h1>
            <PageMetadata :doc="featuredArticle(data)" />
            ...
            <div class="mt-10">
              <NuxtLink :to="featuredArticle(data)._path"
                class="text-base font-medium text-white dark:text-white bg-primary-300 dark:bg-primary-600  px-6 py-3 rounded-md shadow-lg">
                Read more
              </NuxtLink>
            </div>
          </div>

          <!-- extract the photos from the post and put them in a flexbox grid -->
          <!-- <div class="flex flex-wrap justify-center items-center">
            <div v-for="photo in extractPhotos(featuredArticle(data))" :key="photo"
              class="w-20 flex justify-center items-center h-10 border-white border-2">
              <ProseImg :src="photo.props.src" />
            </div>
          </div> -->
        </div>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 xl:gap-16 xl:p-16" ref="articleList">
          <UCard v-for="article in data" :key="article._path" :class="[article.hidden ? 'hidden' : '']">
            <div>
              ...
              <ContentQuery :path="article._path" v-slot="{ data, toc }" find="one">
                <NuxtLink :to="article._path" style="overflow-wrap: break-word;"
                  class="link font-bold text-gray-900 dark:text-gray-100 text-2xl md:text-3xl tracking-tight mb-2 md:mb-4 block">
                  {{
                    article.title
                  }}</NuxtLink>

                <div
                  class="flex justify-between items-center text-gray-600 dark:text-gray-400 text-sm md:text-base mb-2 md:mb-4">
                ...
                <div class="text-gray-600 dark:text-gray-400 text-sm md:text-base mb-2 md:mb-4">
                </div>

                <div class="text-gray-700 dark:text-gray-300 text-md max-w-prose">
                  <div v-if="article.dek" class="font-light my-6">{{ article.dek }}</div>
                  ...
                  <div class="article-toc">
                    <ul class="list-none  flex flex-wrap justify-start items-center">
                      <UButton v-for="link of article.body.toc.links" :key="link.id" color="gray"
                        class="mr-2 mb-2 font-light" size="sm" :to="`${article._path}#${link.id}`">
                        {{ link.text }}
                      </UButton>
                    </ul>
                  </div>
...
import { countPhotos, extractPhotos, filterStrongTags } from "~/helpers";
...
const queryLimit = ref(9);
const querySkip = ref(0);

// element
const articleList = ref(null);

useInfiniteScroll(articleList,
  () => {
    queryLimit.value += 1
  },
  {
