<template>
  <div class="page-metadata pt-3 text-xs">
    <div class="block text-gray-500 font-light text-sm py-2">
      ...
      <span v-if="doc.inprogress" class="">
        <UIcon
          ...
          class="mr-1 text-sm pb-1 text-gray-200 dark:text-gray-700"
        />
        This post is in progress, and updates are expected
      </span>

      <span
        class="mr-4 whitespace-nowrap inline-block text-gray-500"
        v-if="doc.date"
        title="Date created"
      >
        <UIcon name="ant-design:calendar-outlined" class="mr-1 text-sm pb-1" />
        <span class="hidden lg:inline-block"> Started </span>

        {{ formatBlogDate(new Date(doc.date)) }}
      </span>

      <span
        class="hidden lg:inline-block mr-4 whitespace-nowrap text-gray-500"
        v-if="doc.modified"
        title="Date modified"
      >
      ...
      <span
        class="mr-4 whitespace-nowrap inline-block"
        v-if="doc.readingTime?.words > 100"
      >
      ...
      <span
        class="mr-4 whitespace-nowrap inline-block"
        v-if="doc.readingTime?.text !== '1 min read'"
      >
    ...
    <div
      class="strong-tags text-xs font-light text-gray-500 mt-1 italic"
    ...
    >
      ...
      <span
        ...
        :key="tag"
        class="tag inline-block mr-2 mb-2 px-1 py-1 bg-gray-100"
        >{{ tag }}</span
      >
    </div>
  </div>
